...
// ITERATION 3 - Require auth middleware -- middleware check if there is a user in the session
...
router.post('/signup', async (req,res, next)=>{
    try{
        ...
        const {username, email, password} = req.body; // what the user will need to signup
        
        if(!username ||  !email || !password){ // if there is no username or no email or no password then errorMessage
        ...
        if(!regex.test(password)){
            res.status(500).render('auth/signup', {errorMessage: 'Password needs at least 6 characters and must have, at least, 1 uppercase letter'}) // status(500) is the server error
        }
