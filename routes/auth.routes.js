...
// password must be encrypted (bcryptjs)
...
// GET route - will display form for new users to signup
...
router.post("/signup", async (req, res, next) => {
  try {
    ...
    if (!username || !email || !password) {
      res.render("auth/signup", {
        errorMessage: "All fields are mandatory - fill in to signup",
      });
      return;
    }
    ...
    // If the error that was cought it's a Mongoose Validation Error ...
    if (error instanceof mongoose.Error.ValidationError) {
      //  send a HTTP response status of 500 (server-side error)
