...
router.route('/signup')
	...
	.post( (req, res) => {
		...
		User.findOne( { username } )
			.then( (user) => {
				if (user) res.render("signup", { message: "This user already exists!"})
				...
				User.create({ username, password: hashedPwd })
					.then( (user) => res.render("index", { message: "User created!!", user: user }))
					.catch( (err) => res.render("signup", {message: `Oh, no! There's been an error... ${err}`}))
			})
			.catch( (err) => res.render("signup", { message: `Oh, no! There's been an error...${err}`}))
    })
		...
		User.findOne( { username } )
			.then( (user) => {
				...
				if (isPwdCorrect) {
			        ...
			        res.render("userProfile", { user: user, message: "You are logged in!" })
                }
				else res.render("login", { message: "You could not log in!"})

			})
			.catch( (err) => res.render("login", { message: `Oh, no! There's been an error...${err}`}))
